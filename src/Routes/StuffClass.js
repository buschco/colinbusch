  ...
  render() {
    return (
      <div style={{height: '50vh', width: '200px'}} className="content animated fadeIn masonry">
        <button onClick={() => {this._clickHandler()}}>fix</button>
        {this.projects===null ? <MyLoader />: this.state.projects.map((project, i) =>{
          return <Project ready={() => {console.log('ready')}} key={i} title={project.name} description={project.description} links={project.links} media={project.media} />
        })}
      </div>
    )
  }
